    ...
    with open(csv_file, encoding='utf8') as csvfile:
        ...
        for line in reader:
            ...
            data.append({
                ...
                'price': int(line['Цена']),
                'place': line['Место'],
                'date': datetime.datetime(year=2020, month=int(date[1]), day=int(date[0]))
            })
...
def find_by_date(start, end, db):
    ...
    concerts_by_date = db.concerts.find({'$and': [
        {'date': {'$gte': datetime.datetime(year=2020, month=int(start_date[1]), day=int(start_date[0]))}},
        {'date': {'$lte': datetime.datetime(year=2020, month=int(end_date[1]), day=int(end_date[0]))}}
    ]})
